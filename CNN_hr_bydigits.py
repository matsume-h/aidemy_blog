# os,numpy,PIL,datasets,utility, tensorflow のインポート
import os
import numpy as np
...
layers = [
    ...
    cnn.Pooling((1, 2, 2, 1)),
    cnn.Flatten(),
    cnn.Dense(2 * 2 * 20, 10, tf.nn.softmax)
]
...
for epoch in range(n_epochs):
    ...
    pred_y, valid_cost = sess.run([valid, cost], feed_dict={
                                  x: valid_X, t: valid_y})
    #訓練過程を出力
    if epoch % 100 == 99:
        print('EPOCH:: %i, 誤差: %.3f, F1値: %.3f' % (epoch + 1, valid_cost,
                                                   f1_score(np.argmax(valid_y, 1).astype('int32'), pred_y, average='macro')))
"""
------------------------------------------------------------------------------
"""
#テストデータを識別
